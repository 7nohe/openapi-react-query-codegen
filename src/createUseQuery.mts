...
export const createApiResponseType = ({ className, methodName }: { className: string; methodName: string }) => {
    ...
    const awaitedResponseDataType = ts.factory.createTypeReferenceNode(ts.factory.createIdentifier('Awaited'), [
        ts.factory.createTypeReferenceNode(ts.factory.createIdentifier('ReturnType'), [
            ts.factory.createTypeQueryNode(
                ts.factory.createQualifiedName(ts.factory.createIdentifier(className), ts.factory.createIdentifier(methodName)),
                undefined
            ),
        ]),
    ]);
    ...
    const apiResponse = ts.factory.createTypeAliasDeclaration(
        ...
        ts.factory.createIdentifier(`${capitalizeFirstLetter(className)}${capitalizeFirstLetter(methodName)}DefaultResponse`),
        undefined,
...
export function getRequestParamFromMethod(method: MethodDeclaration, pageParam?: string) {
    ...
    return ts.factory.createParameterDeclaration(
        ...
        ts.factory.createObjectBindingPattern(
            params.map((refParam) =>
                ts.factory.createBindingElement(undefined, undefined, ts.factory.createIdentifier(refParam.name), undefined)
            )
        ),
        undefined,
        ...
        areAllPropertiesOptional ? ts.factory.createObjectLiteralExpression() : undefined
    );
}
...
export function createReturnTypeExport({
    ...
    return ts.factory.createTypeAliasDeclaration(
        [ts.factory.createModifier(ts.SyntaxKind.ExportKeyword)],
        ts.factory.createIdentifier(`${capitalizeFirstLetter(className)}${capitalizeFirstLetter(methodName)}QueryResult`),
        [
 ...
 * QueryKey
 */
export function createQueryKeyExport({ className, methodName, queryKey }: { className: string; methodName: string; queryKey: string }) {
...
export function hookNameFromMethod({ method, className }: { method: MethodDeclaration; className: string }) {
    const methodName = getNameFromMethod(method);
    return `use${className}${capitalizeFirstLetter(methodName)}`;
}

export function createQueryKeyFromMethod({ method, className }: { method: MethodDeclaration; className: string }) {
...
export function createQueryHook({
    ...
    if (queryString === 'useInfiniteQuery' && (pageParam === undefined || nextPageParam === undefined)) {
    ...
    const responseDataTypeIdentifier = responseDataTypeRef.typeName as ts.Identifier;
                ...
                ts.factory.createVariableDeclaration(
                    ...
                    ts.factory.createArrowFunction(
                        ...
                        ts.factory.createNodeArray([
                            isInfiniteQuery
                                ? ts.factory.createTypeParameterDeclaration(
                                      ...
                                      TData,
                                      undefined,
                                      ts.factory.createTypeReferenceNode(ts.factory.createIdentifier('InfiniteData'), [
                            ...
                            ts.factory.createParameterDeclaration(
                                ...
                                ts.factory.createTypeReferenceNode(ts.factory.createIdentifier('Omit'), [
                                    ts.factory.createTypeReferenceNode(
                                        ts.factory.createIdentifier(isInfiniteQuery ? 'UseInfiniteQueryOptions' : 'UseQueryOptions'),
                                        [ts.factory.createTypeReferenceNode(TData), ts.factory.createTypeReferenceNode(TError)]
                                    ),
                        ...
                        ts.factory.createCallExpression(
                            ...
                            isInfiniteQuery ? [] : [ts.factory.createTypeReferenceNode(TData), ts.factory.createTypeReferenceNode(TError)],
                            [
...
export const createUseQuery = ({ className, method, jsDoc }: MethodDescription, pageParam: string, nextPageParam: string) => {
    const methodName = getNameFromMethod(method);
    const queryKey = createQueryKeyFromMethod({ method, className });
    ...
    const infiniteHookWithJsDoc = infiniteQueryHook ? addJSDocToNode(infiniteQueryHook, jsDoc) : undefined;
...
function queryKeyFn(queryKey: string, method: MethodDeclaration): ts.Expression {
            ...
            ts.factory.createArrowFunction(
                ...
                ts.factory.createPropertyAccessExpression(
                    ts.factory.createParenthesizedExpression(
                        ts.factory.createAsExpression(
                            ts.factory.createIdentifier('response'),
                            ts.factory.createTypeReferenceNode(ts.factory.createIdentifier(`{ ${nextPageParam}: number }`))
                        )
                    ),
