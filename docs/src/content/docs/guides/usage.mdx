...
The factory takes a third argument for the pagination overrides, so a custom scheme can be defined once and reused everywhere the factory is:

```tsx
const petPages = findPaginatedPetsInfiniteOptions({}, undefined, {
  initialPageParam: "",
